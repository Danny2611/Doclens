...
export function assertProcessingRunTransition(
  from: ProcessingRunStatus,
  to: ProcessingRunStatus,
): void {
  if (!canTransitionProcessingRun(from, to))
    throw new Error(`Invalid processing run transition: ${from} -> ${to}.`);
}
export function isProcessingRunActive(status: ProcessingRunStatus): boolean {
  return status === 'PENDING' || status === 'RUNNING';
}
export function isProcessingRunTerminal(status: ProcessingRunStatus): boolean {
