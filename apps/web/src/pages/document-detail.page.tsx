...
export function DocumentDetailPage(): JSX.Element {
  ...
  if (!documentId)
    return (
      <main className="p-8">
        <p role="alert">Document ID không hợp lệ.</p>
      </main>
    );
  if (query.isPending)
    return (
      <main className="p-8">
        <p role="status">Đang tải tài liệu…</p>
      </main>
    );
  if (query.isError) {
    ...
    return (
      <main className="p-8">
        <p role="alert">
          {notFound ? 'Không tìm thấy tài liệu.' : 'Không thể tải tài liệu. Hãy thử lại.'}
        </p>
        <Link to="/">Quay lại thư viện</Link>
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto max-w-2xl p-8">
      ...
      <h1 className="mt-6 text-3xl font-bold">{document.originalName}</h1>
      <div className="mt-4">
      ...
      <dl className="mt-6 grid gap-3 text-sm">
        ...
        {document.pageCount !== null ? (
          <div>
            <dt className="font-medium">Số trang</dt>
            <dd>{document.pageCount}</dd>
          </div>
        ) : null}
        <div>
          <dt className="font-medium">Tạo lúc</dt>
